...
#		export PATH="/c/Program Files (x86)/NASM":$PATH

# ./make-vs2015-x86_64-env.bat OR ./make-vs2015-x86-env.bat
# <execute each export command>

# !!! For x86 replace -lsnappy64 with -lsnappy32 in configure file

# make clean && bash ./configure_win_x86_msvc.sh
# time make -j9 && make install
# Note: use just "make" after "make clean" if you see any errors 

# !!! For x86 replace back to -lsnappy64 in configure 
...
./configure   --prefix=builds/win_x86_msvc \
		...
		--enable-avcodec --enable-avformat --enable-swresample --enable-swscale --disable-avfilter \
		...
		--extra-ldflags="-LIBPATH:${REPO_ROOT}/3rdparty/gdeflate/lib/x86" \
        --extra-cflags="-I ${REPO_ROOT}/3rdparty/ffmpeg/libavcodec/bc7e/build/include" \
		--extra-ldflags="-LIBPATH:${REPO_ROOT}/3rdparty/ffmpeg/libavcodec/bc7e/build/lib/x86"
